package com.mosaicsheep.huffmancodeing.zipfile;

import java.io.*;
import java.util.*;

/**
 * 在HuffmanCodingRefactored的基础上，对文件进行压缩/解压
 *
 * @author LeslieTang
 * @version 2019/12/17
 */
public class HuffmanCodingZipFile {
    public static void main(String[] args) {
        // 测试压缩文件
        String srcFile = "/Users/leslietang/Documents/backup/computer science/java/0.Algorithm/Algorithm/src/1.png";
        String dstFlie = "/Users/leslietang/Documents/backup/computer science/java/0.Algorithm/Algorithm/src/1.zip";
        zipFile(srcFile, dstFlie);

        // 测试解压缩文件
        srcFile = "/Users/leslietang/Documents/backup/computer science/java/0.Algorithm/Algorithm/src/1.zip";
        dstFlie = "/Users/leslietang/Documents/backup/computer science/java/0.Algorithm/Algorithm/src/2.png";
        unzipFile(srcFile, dstFlie);
    }

    /**
     * 对文件进行解压
     *
     * @param srcFile 待解压文件的全路径
     * @param dstFile 解压后文件的存放路径
     */
    public static void unzipFile(String srcFile, String dstFile) {
        try (InputStream is = new FileInputStream(srcFile); ObjectInputStream ois = new ObjectInputStream(is); OutputStream os = new FileOutputStream(dstFile);) {

            byte[] zippedBytes = (byte[]) ois.readObject();
            Map<Byte, String> huffmanCodes = (Map<Byte, String>) ois.readObject();

            byte[] unzippedBytes = decode(huffmanCodes, zippedBytes);
            os.write(unzippedBytes);
    ...
    public static void zipFile(String srcFile, String dstFile) {
        try (InputStream is = new FileInputStream(srcFile); OutputStream os = new FileOutputStream(dstFile); ObjectOutputStream oos = new ObjectOutputStream(os);) {
            byte[] bytes = new byte[is.available()];// 创建和源文件大小一样的字节数组
            is.read(bytes);// 将源文件的内容写入到byte数组中
            byte[] zippedBytes = huffmanZip(bytes);// 进行霍夫曼编码压缩

            oos.writeObject(zippedBytes);// 以对象流的方式写入霍夫曼编码，是为了以后恢复源文件时使用
            oos.writeObject(huffmanCodes);// 同时，也要把霍夫曼编码表写入压缩文件
